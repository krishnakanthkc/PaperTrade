...
if 'trade_log' not in st.session_state:
    try:
        # Fetch data from the "Ledger" worksheet
        df = conn.read(worksheet="Ledger", usecols=list(range(5)))
...
def save_ledger():
    ...
    conn.update(worksheet="Ledger", data=st.session_state.trade_log)
...
if not strat_rets.empty:
    ...
    # MOBILE OPTIMIZATION: 2x2 Grid for metrics
    m1, m2 = st.columns(2)
    m3, m4 = st.columns(2)
